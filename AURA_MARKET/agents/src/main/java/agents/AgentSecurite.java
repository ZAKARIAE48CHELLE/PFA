...
/**
 * Agent JADE responsable de la vérification de sécurité.
 */
public class AgentSecurite extends Agent {
            ...
            public void action() {
                ...
                if (msg != null) {
                    try {
                        ...
                        String type = req.has("type") ? req.get("type").asText() : "OFFRE"; // Default to OFFRE for AURA_MARKET compatibility
    ...
    private ObjectNode analyserOffre(JsonNode req) {
        try {
            ...
            // Préparation du payload pour le service ML
            ...
            // "NON ACCEPTABLE" dans le ML devient "SUSPECT" pour l'agent (cohérence avec le frontend existant)
            boolean isSuspect = "NON ACCEPTABLE".equalsIgnoreCase(statutMl);
            response.put("statut", isSuspect ? "SUSPECT" : "ACCEPTABLE");
            response.put("isSuspect", isSuspect); // For backwards compatibility
            ...
            if (isSuspect) {
                ...
                if (mlResponse.has("propositions") && mlResponse.get("propositions").isArray()) {
                    for (JsonNode prop : mlResponse.get("propositions")) {
                        // Les propositions sont des pourcentages de remise (ex: 15)
            ...
            // Fallback sur l'ancienne logique simplifiée en cas d'erreur API
    ...
    private ObjectNode verifierCommentaire(JsonNode node) {
        ...
        try {
            ...
            try {
                // Backward compatibility with old ml_api schema.
